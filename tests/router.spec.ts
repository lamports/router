  ...
  describe("MINTING NFT", async() => {
    ...
    it("Should allow transfer sols to the router account", async() => {
      try {
        ...
        //const before:RouterData = await getRouterData(program, routerAccount);

         // console.log(before.config.goLiveDate.toString())
        await program.rpc.updateConfig({
          price : null,
          goLiveDate : new anchor.BN(secondsSinceEpoch- 10000000),
          uuid : null,
          itemsAvailable : null
