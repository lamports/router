...
export const getDefaultAnchorWorkspace = (programType: string): Workspace => {
  const provider: anchor.Provider = anchor.Provider.local();
  let program = null;

  if (programType === "router") {
    program = anchor.workspace.Router;
  } else if (programType === "vault") {
...
export const getSigner1Wallet = () => {
  return anchor.web3.Keypair.fromSecretKey(
    new Uint8Array(
      JSON.parse(
        require("fs").readFileSync(process.env.SIGNER_1_WALLET, "utf8")
      )
    )
  );
};
...
export const getSigner2Wallet = () => {
  return anchor.web3.Keypair.fromSecretKey(
    new Uint8Array(
      JSON.parse(
        require("fs").readFileSync(process.env.SIGNER_2_WALLET, "utf8")
      )
    )
  );
};

export const getCustomWorkspace = (
  wallet: Keypair,
  idlPath: string,
  programIdStr: string
): Workspace => {
