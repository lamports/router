...
describe("MINTING NFT", async() => {
    ...
    if(JSON.parse(process.env.USE_DEFAULT_WORKSPACE)){
      ...
      routerWorkspace = getCustomWorkspace(signer2Wallet, process.env.ROUTER_IDL_PATH, process.env.ROUTER_PROGRAM_ID);
      vaultWorkspace = getCustomWorkspace(signer2Wallet, process.env.VAULT_IDL_PATH, process.env.VAULT_PROGRAM_ID);
